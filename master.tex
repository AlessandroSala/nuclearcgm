...
\end{equation}
Here, coordinate indeces $x\text{ and } y$, are used to denote partial derivatives of $\psi$ with respect to $x$ and $y$ respectively.
...
\end{equation}
We can now write $\psi_{yy, i}$ as a function of $\psi_i(y), y, E, x_i$ and apply the finite differences method on $y$, which yields:
\begin{equation}
    \frac{\psi_{i, j+1}+\psi_{i, j-1}-2\psi_{i, j}}{h_y^2} + \frac{\psi_{i+1, j}+\psi_{i-1, j}-2\psi_{i,j}}{h_x^2} = f(\psi_{i, j}, x_i, y_j, E)\label{eq:finite_diff_2nd_pde}
\end{equation}
...
\\The next task will be to find a way to express $\ref{eq:hypermatrix_2d}$ as a linear system of equations, by finding a single index $\mu$ that encapsulates the relation for all pairs $(i, j)$, with the constraint that the resulting matrix is symmetric.
\\Formally
\begin{equation}
    F:(i,j)\mapsto \mu \text{ such that }\mu = F(i, j),\ \mu' = F(i',j')\implies\ A_{\mu\mu'} = A_{\mu'\mu}
\end{equation}
\subsubsection{Indeces parametrization}
A possible map $(i, j) \mapsto \mu$ is
\begin{equation}
F(i, j) = \mu = i + N_x j \label{eq:indeces_parametrization}
\end{equation}
...
\begin{equation}
\sum_{\mu'=0}^{N_x N_y}A_{\mu\mu'}\psi_{\mu'} = E\psi_\mu
\end{equation}
Which can be diagonalized to find the solutions to the eigenvalue problem in multiple dimensions. Moreover, an approximate solution can be found using cgm, since the resulting matrix will be symmetric.
