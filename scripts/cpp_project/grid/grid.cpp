#include "grid.hpp" // Include the class declaration
#include <stdexcept> // For std::invalid_argument
#include <string>    // For std::to_string in error message

// Constructor implementation
Grid::Grid(size_t n, double a) : n_points_(n), a_(a), h_(0.0) // Initialize members
{
    if (n_points_ < 2) {
        throw std::invalid_argument("Grid error: n must be >= 2. Received n = " + std::to_string(n_points_));
    }
...
size_t Grid::get_n() const noexcept {
    return n_points_;
}

double Grid::get_a() const noexcept {
    return a_;
}

double Grid::get_h() const noexcept {
    return h_;
}

const std::vector<double>& Grid::get_xs() const noexcept {
    return xs_;
}

const std::vector<double>& Grid::get_ys() const noexcept {
    return ys_;
}

const std::vector<double>& Grid::get_zs() const noexcept {
    return zs_;
}
