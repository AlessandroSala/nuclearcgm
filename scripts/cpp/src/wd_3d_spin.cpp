...
int main(int argc, char **argv) {
  ...
  pots.push_back(make_shared<DeformedSpinOrbitPotential>(
      DeformedSpinOrbitPotential(V0, Radius(0, r_0_so, A_val), 0.7)));
  pots.push_back(make_shared<DeformedWoodsSaxonPotential>(
      DeformedWoodsSaxonPotential(V0, Radius(0, r_0, A_val), 0.7, A_val, input.getZ(), input.get_json()['kappa'])));
  ...
  eigenpair = gcgm_complex_no_B(
      hamNoKin.buildMatrix(),
  ...
  for (int i = 0; i < calc.hfCycles; ++i) {
    vector<shared_ptr<Potential>> pots;
    VectorXd rho = Wavefunction::density(eigenpair.first, grid);
    cout << "density gen" << endl;
    Matrix<double, -1, 3> nablaRho = Operators::gradNoSpin(rho, grid);
    cout << "nabla rho gen" << endl;
    VectorXd nabla2rho = Operators::divNoSpin(nablaRho, grid).real();
    cout << "nabla^2 rho gen" << endl;
    VectorXd tau = Wavefunction::kineticDensity(eigenpair.first, grid);
    cout << "tau gen" << endl;
    Matrix<std::complex<double>, -1, 3> J = Wavefunction::soDensity(eigenpair.first, grid);
    cout << "J gen" << endl;
    VectorXcd divJ = Operators::divNoSpin(J, grid);
    cout << "div J gen" << endl;
    Matrix<double, -1, 3> W = 0.75*input.skyrme.W0*nablaRho;

    cout << "Quantum gen" << endl;

    pots.push_back(make_shared<LocalKineticPotential>(make_shared<Mass>(m)));
    pots.push_back(make_shared<NonLocalKineticPotential>(make_shared<Mass>(m)));
    pots.push_back(make_shared<SkyrmeU>(make_shared<VectorXd>(rho), make_shared<VectorXd>(nabla2rho), make_shared<VectorXd>(tau), make_shared<VectorXcd>(divJ), input.skyrme.t0, input.skyrme.t1, input.skyrme.t2, input.skyrme.t3, input.skyrme.W0));
    pots.push_back(make_shared<SkyrmeSO>(make_shared<Matrix<double, -1, 3>>(W)));
    
    cout << "Skyrme gen" << endl;

    Hamiltonian skyrmeHam(make_shared<Grid>(grid), pots);
    auto newEigenpair = gcgm_complex_no_B(
        skyrmeHam.buildMatrix(),
        eigenpair.first, calc.nev, 35 + 0.01, calc.cycles, 1.0e-3, 20, 1.0e-4 / (calc.nev), false, 1);

    cout << "Residual: " << (newEigenpair.first - eigenpair.first).norm() << endl;

    eigenpair = newEigenpair;
    
  }
  return 0;
  for (int i = 0; i < eigenpair.first.cols(); ++i) {
    auto L2 = Operators::L2(eigenpair.first.col(i), grid)
                  .dot(eigenpair.first.col(i)) /
              (h_bar * h_bar);
    cout << "L2: " << L2.real() << " | ";
    // cout << ( Operators::LS(eigenpair.first.col(i),
    // grid).norm())/(h_bar*h_bar)<< " | "; cout << "J2: " <<
    // eigenpair.first.col(i).adjoint() * Operators::J2(eigenpair.first.col(i),
    // grid)/(h_bar*h_bar)<< endl;
    cout << "J2: "
         << (eigenpair.first.col(i).adjoint() *
             Operators::J2(eigenpair.first.col(i), grid))
                    .norm() /
                (h_bar * h_bar)
         << " | ";
    auto par = (eigenpair.first.col(i).adjoint() *
                Operators::P(eigenpair.first.col(i), grid));
    cout << "P: " << (par / par.norm()).real() << " | ";
    cout << "mz: "
         << Operators::Jz(eigenpair.first.col(i), grid).norm() / (h_bar)
         << endl;
  }
  ...
  for (double Beta = -0.5; Beta < 0.55; Beta += 0.1) {
    ...
    pots.push_back(make_shared<DeformedSpinOrbitPotential>(
        DeformedSpinOrbitPotential(V0, Radius(Beta, r_0_so, A_val), 0.7)));
    pots.push_back(make_shared<DeformedWoodsSaxonPotential>(
        DeformedWoodsSaxonPotential(V0, Radius(Beta, r_0, A_val), 0.7, A_val, input.getZ(), input.get_json()['kappa'])));
