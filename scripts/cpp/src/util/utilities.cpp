...
Utilities::solve(const ComplexSparseMatrix &matrix, GCGParameters &calc,
  ...
  return gcgm_complex_no_B_lock(matrix, guess, guess.cols(),
                                0.0, calc.maxIter, calc.tol, calc.steps,
                                (calc.cgTol), false);
}
...
void Utilities::skyrmeHamiltonian(std::vector<std::shared_ptr<Potential>> &pots,
  ...
  pots.push_back(
      make_shared<SkyrmeU>(input.skyrme, t, make_shared<IterationData>(data)));
  pots.push_back(make_shared<NonLocalKineticPotential>(
      make_shared<IterationData>(data), t));
  pots.push_back(
      make_shared<LocalKineticPotential>(make_shared<IterationData>(data), t));
  if (input.spinOrbit)
    pots.push_back(make_shared<SkyrmeSO>(make_shared<IterationData>(data), t));
