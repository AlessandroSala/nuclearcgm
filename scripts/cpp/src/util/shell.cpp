...
double Shell::l() {
  ...
  auto L2 = Operators::integral(
      (*psi * (Operators::L2(*psi, *grid).adjoint())).diagonal(), *grid);
  return round(momFromSq(L2.real() / (h_bar * h_bar)));
}
double Shell::j() {
  using namespace nuclearConstants;
  auto J2 = Operators::integral(
      (*psi * (Operators::J2(*psi, *grid).adjoint())).diagonal(), *grid);
  return roundHI(momFromSq(J2.real() / (h_bar * h_bar)));
}
double Shell::mj() {
  using namespace nuclearConstants;
  auto mj = roundHI(
      sqrt(Operators::integral(
               (*psi *
                (Operators::Jz(Operators::Jz(*psi, *grid), *grid)).adjoint())
                   .diagonal(),
               *grid)
               .real()) /
      (h_bar));
