...
X2MY2Constraint::X2MY2Constraint(double mu20) : mu20(mu20), C(0.01), lambda(0.0), firstIter(true)
{
  residuals.clear();
}
Eigen::VectorXd X2MY2Constraint::getField(IterationData *data)
{
  ...
  // std::cout << "XCM: " << integral((VectorXd)(x.array()*rho.array()), grid) << std::endl;
  // std::cout << "YCM: " << integral((VectorXd)(y.array()*rho.array()), grid) << std::endl;
